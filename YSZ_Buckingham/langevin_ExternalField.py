"""Langevin dynamics class. + External field efield = [Ex, Ey, Ez]"""
...
class Langevin(MolecularDynamics):
    ...
    def __init__(self, atoms, timestep, efield, temperature=None, friction=None,
                 fixcm=True, *, temperature_K=None, trajectory=None,
                 logfile=None, loginterval=1, communicator=world,
                 rng=None, append_trajectory=False, out_field=False):
        """
        Parameters:
        ...
        ### added for external field ###
        efield: external, static electric field (numpy array: [Ex,Ey,Ez])
            unit: V/angstrom     
        ###
    ...
    def step(self, forces=None):
        ...
        if forces is None:
            ...
            aa=atoms.get_initial_charges()
            #aa=np.ravel(aa)
            naa=aa.shape[0]
            aa=repmat(aa,3,1).T
            efields=repmat(efield.T,naa,1)
            f_ex = aa * efields
            
            if(out_field):
                np.savetxt("normalforce_efield.txt",forces)
                np.savetxt("externalforce_efield.txt",f_ex)
                np.savetxt("totalforce_efield.txt",forces+f_ex)
